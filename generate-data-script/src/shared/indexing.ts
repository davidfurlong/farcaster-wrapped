...
export function formatMerkleApiCast(
  ...
  // if (
  //   cast.parentHash &&
  //   (!cast.parentAuthor?.fid || !cast.parentAuthor?.username)
  // ) {
  //   console.log("Cast had parentHash but not parentAuthor ", cast);
  // }
  return {
    type: "text-short",
    ...
    recasted_cast_hash: cast.text.startsWith("recast:farcaster://casts/")
      ...
      : null,
    reply_parent_hash: cast.parentHash || null,
    thread_hash: cast.threadHash || cast.hash,
    hash: cast.hash,
    ...
    reply_to_data: null,
    recast_data: null,
    // can't have a partial property
    // deleted:
    //   // cleanUserActivity adds this
    //   (cast as any).deleted || cast.text.startsWith("delete:farcaster://")
    //     ? true
    //     : false,
